...
with model:
    ...
    result = nengo.Ensemble(100, dimensions=1, radius = 15)
...
with model:
    ...
    nengo.Connection(input_neuron, result, function=compare_neurons)

## Add probes to view data from each ensemble and output
with model:
    input_probe = nengo.Probe(input_neuron)
    result_probe = nengo.Probe(result,synapse=0.01)
...
data_sz = sim.data[result_probe].shape 
correct_ans_arr = np.repeat(correct_ans,data_sz[0])

## Plot the results!
# Plot the input signals and decoded ensemble values
t = sim.trange()
plt.figure()
plt.plot(sim.trange(), sim.data[result_probe], label="Neuron Output")
plt.plot(sim.trange(), correct_ans_arr, label="Right Answer")
plt.legend()
plt.ylim(0, 8)
plt.xlabel("time [s]")
plt.title("Return Greatest Value")
plt.show()
